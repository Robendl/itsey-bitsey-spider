...
to setup
  ...
  ; initializing flies
   create-flies number-of-flies [
    ...
    set color black
    set shape "butterfly"
    set energy starting-energy-fly
  ]
  reset-ticks
end
...
to eat-flies
  if any? flies-here [
    ...
    set energy energy + [energy] of target
    ask target [
      die
    ]
...
to reproduce
  if energy > reproduction-threshold [
    ...
    hatch 1 [set energy starting-energy-spider]
  ]
end

to check-if-fly-dead
  if energy < 0 [
    die
  ]
end
...
to check-if-spider-dead
  if energy < 0 [
    ...
    let neighboring-web (list)
    ask patch-set web-patches [
      ask neighbors4 with [pcolor = white] [
        if not member? self current-web [
          set neighboring-web lput self neighboring-web
        ]
      ]
    ]
    let web-is-neighbor false
    ask spiders [
      if energy > 0 [
        ask patch-set web-patches [
          if member? self neighboring-web [
            set web-is-neighbor true
          ]
        ]
        if web-is-neighbor [
          set web-patches lput current-web web-patches
          set web-is-neighbor false
          stop
        ]
      ]
    ]
...
1269
108
Blue indicates free space; the sky\nGrey indicates the spawning point of the flies; a thrashcan\nGreen indicates the goal that the flies have to reach in order to be free
11
0.0
1
